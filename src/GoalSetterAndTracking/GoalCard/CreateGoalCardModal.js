...
class CreateGoalCardModal extends React.Component {
  ...
  componentDidMount = () => {
    ...
    if (this.state == null || this.state == undefined)
      this.initializeInputFields(false);
        ...
        if (this.props.items.hasOwnProperty(i)) {
          inputFields.push(
            <TextInput
              ...
              style={styles.textInput}
              onChangeText={text => this.handleInputChange(i, text)}
              defaultValue={this.props.items[i].information}
            />,
          );
        } else {
          inputFields.push(
  ...
  addInputField = key => {
    ...
    this.setState({
      inputFields: inputFieldsNew,
      lastUsedKey: this.state.lastUsedKey + 1,
    });
  };
  ...
  handleInputChange = (key, text) => {
    ...
    if (!(key in this.state.items)) {
      ...
      let itemsReplacement = Object.assign({}, itemsTrimmed, {
        [key]: replacement,
      });
      this.setState({
        items: itemsReplacement,
      ...
      <View style={styles.viewContainer}>
        <ScrollView>
          ...
          <Text style={styles.description}>
            ...
            care of our mental, emotional, or physical health. It can be as
            simple as taking a nap, going to a yoga class, or reading a good
            book. How are you going to engage in self-care this week?
          </Text>
          ...
          <View style={styles.saveButtonContainer}>
            <Button
              disabled={Object.entries(this.state.items).length === 0}
              ...
              onPress={() => {
                ...
                if (ed == null || sd == null) {
                  ToastAndroid.show(
                    'You must pick both a start date and an end date.',
                    ToastAndroid.SHORT,
                  );
                } else if (
                  new Date(ed).setHours(0, 0, 0, 0) <
                  new Date(sd).setHours(0, 0, 0, 0)
                ) {
                  ToastAndroid.show(
                    'You must pick an end date equal to or after your start date before saving.',
                    ToastAndroid.SHORT,
                  );
                } else {
                  if (!this.props.edit) {
